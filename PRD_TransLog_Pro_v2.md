...
| Champ | Valeur |
|---|---|
| Version | 2.0 — Post-Critique Architecturale |
...
|---|---|---|
| Client | Web / Mobile App | Recherche trajet, Réservation, Paiement Mobile Money, Tracking temps réel |
...
| Chauffeur | App Mobile | Roadbook, Checklists, Incidents, Pauses, SOS |
| Mécanicien | App Garage | Alertes pannes, Rapports intervention, Remise en service |
| Planificateur | Admin Web | Lignes, Routes, Affectation Flotte/Personnel, Yield Management |
| Administrateur Tenant | Admin Web | Workflow config, Audit financier, Permissions, Tarifs |
...
| module | `iam` · `workflow` · `ticket` · `parcel` · `trip` · `fleet` · `pricing` · `cashier` · `sav` · `maintenance` · `manifest` · `traveler` · `luggage` · `shipment` · `session` · `user` · `integration` · `settings` · `module` · `route` · `bus` · `fleet` |
| action | `create` · `read` · `update` · `delete` · `scan` · `cancel` · `approve` · `report` · `check` · `open` · `close` · `transaction` · `deliver` · `claim` · `manage` · `config` · `override` · `install` · `revoke` · `verify` · `track` · `weigh` · `group` · `layout` · `yield` · `audit` · `setup` |
...
### VI.2 API Endpoints (Complets, v2.0)
...
*Fin du PRD TransLog Pro v2.0*
*Révision basée sur critique architecturale complète — Avril 2026*
