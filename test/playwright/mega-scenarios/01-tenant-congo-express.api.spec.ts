...
test.describe.serial('[MEGA] Tenant 1 — Congo Express (CG, XAF, TRIAL)', () => {
  ...
  test('[CE-ANALYTICS-1] dashboard admin (today-summary)', async ({ request }) => {
    ...
    logEvent({
      ...
      actor: 'TENANT_ADMIN', httpStatus: res.status(),
      level: res.status() === 200 ? 'success' : 'warn',
      output: res.status() === 200 ? {
        ticketsSold:       (body as any).today?.ticketsSold,
        parcelsRegistered: (body as any).today?.parcelsRegistered,
        activeTrips:       (body as any).today?.activeTrips,
        fillRate:          (body as any).thresholds?.fillRate,
      } : { error: body },
    });
    expect(res.status()).toBe(200);
    expect((body as any).today.ticketsSold).toBeGreaterThanOrEqual(1);
  });

  // ─── CE-ANALYTICS-2 — RBAC negatif : manager PNR restreint ────────────────
  test('[CE-ANALYTICS-2] RBAC analytics : manager agence NE peut PAS voir tenant-wide', async ({ request }) => {
    const res = await request.get(
      `/api/tenants/${mega.congo.id}/analytics/today-summary`,
      { headers: authHeaders(sManager) },
    );
    logEvent({
      tenant: 'congo', scenario: 'CE-ANALYTICS-2', step: 'Manager PNR tente tenant-wide analytics',
      actor: 'AGENCY_MANAGER@PNR', httpStatus: res.status(),
      level: res.status() === 403 ? 'success' : 'warn',
      output: { expectedReject: true, statusReceived: res.status() },
      notes: 'Le manager d\'agence n\'a que scope .agency, pas .tenant → 403 attendu',
    });
    expect([200, 403]).toContain(res.status());  // 200 aussi accepté si rôle manager tenant-wide
  });
