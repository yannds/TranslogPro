...
test.describe.serial('[MEGA] Tenant 2 — Sahel Transport (SN, XOF, PAID)', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    const quaiKey    = Object.keys(sahel.users).find(k => sahel.users[k].roleName === 'QUAI_AGENT')!;
  ...
  test('[ST-WED-2] Mercredi 09h30 : PANNE MOTEUR sur bus SN-AB-101 à Mboro', async () => {
    ...
    await prisma.trip.update({
      where: { id: wedTripId },
      data:  { status: 'SUSPENDED', suspendReason: 'Panne moteur — thermostat refroidissement' },
    });
