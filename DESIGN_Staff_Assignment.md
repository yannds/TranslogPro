# DESIGN — Staff = enveloppe RH + Affectations métier (Assignments)

**Statut :** Proposition validée — option β (Staff reste, Assignments introduits)
...
### 3.1 Option β retenue : `Staff` reste, `Assignment` arrive
...
`Assignment` porte le **poste occupé** : rôle métier, agence d'opération, dates, dispo, licence spécifique.
...
- `Assignment.agencyId` renseigné → opère dans **cette agence**
- `Assignment.agencyId = null` → opère **tenant-wide** (tous sites)

Pas de join-table `AssignmentAgency` pour l'instant : si un user couvre 3 agences sur 5, on crée **3 lignes d'affectation** (une par agence), avec le même rôle. C'est plus verbeux mais plus simple à requêter, plus simple à clore individuellement, et plus proche de la réalité (un contrôleur peut être suspendu de l'agence A sans l'être de B).

Si un jour cette verbosité devient insupportable, on introduira `AssignmentAgency`. Pour l'instant, YAGNI.
...
**Supprimés :**
- `role: String` → déplacé sur `Assignment.role`
- `licenseData: Json` → déplacé sur `Assignment.licenseData` (chaque rôle a ses propres licences)
- `isAvailable: Boolean` → déplacé sur `Assignment.isAvailable` (dispo par rôle)
- `totalDriveTimeToday: Float` → déplacé sur `Assignment` (pertinent uniquement côté chauffeur)
...
- Script de data migration : pour chaque `Staff` existant → créer 1 `Assignment` avec les valeurs actuelles (`role`, `agencyId`, `licenseData`, `isAvailable`, `totalDriveTimeToday`, `startDate=Staff.createdAt`)
...
### Phase 2 — Services lisent les Assignments *(invisible UI)*
- Réécriture `StaffService.findAll(role)` → join sur `Assignment` avec filtres
...
DTOs : `CreateAssignmentDto`, `UpdateAssignmentDto`, `AssignmentQueryDto`.
...
- Les modules métier (Chauffeur, Contrôleur…) continuent de fonctionner — ils lisaient `Staff.role` via les services, qui lisent maintenant `Assignment`
...
- Tous les tests qui seedent directement `staff.role` sans passer par `Assignment`
...
- **`TECHNICAL_ARCHITECTURE.md`** : nouveau diagramme User ↔ Staff ↔ Assignment ↔ Agency + note sur la séparation IAM/métier
- **`RESULTATS.md`** : nouvelle entrée datée retraçant la refonte
- **`TEST_STATUS.md`** : couverture mise à jour
- **Ce fichier (`DESIGN_Staff_Assignment.md`)** : ajout d'une section « État final » en haut, pour dire « Implémenté le YYYY-MM-DD, commits XXX..YYY »
...
|---|---|---|
| `prisma/schema.prisma` | ajout `Assignment`, modif `Staff` | 1 |
| `prisma/seeds/dev.seed.ts` | seed via `Assignment` | 1 |
...
### 9.3 Tests à adapter
- Tous les tests qui appellent `staff.create({ role: 'DRIVER' })` → devront créer Staff puis Assignment
- `test/helpers/*.ts` : helpers de seed à étendre avec `createStaffWithAssignment()`
...
- [ ] `DESIGN_Staff_Assignment.md` (ce fichier) — état final
